...
def formatting_fix(str):
    length = len(str)
    context_str = "Context:"
    head_str = "Head Entity:"
    tail_str = "Tail Entity:"
    index_cont = str.lower().index(context_str.lower())
    index_head = str.lower().index(head_str.lower())
    index_tail = str.lower().index(tail_str.lower())
    index1=index_cont + len(context_str)-1
    index2=index_head-2
    indexd=index_tail-2
    index3=index_head+len(head_str)-1
    index4=index_tail+len(tail_str)-1
    str = str[:index1] + ' ' + str[index1:index2] + ' ' + str[index2:index3]+ ' ' +str[index3:indexd]+' '+ str[indexd:index4] + ' ' + str[index4:length-1] + ' .'
    return str
